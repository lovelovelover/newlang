...
$(".searchTxt").click(function(){
	//alert($(this).children(".showList"))
	if(num==0){
		$(".scarchBox").children(".list").slideDown();
		num = 1;
	}else{
		$(".scarchBox").children(".list").slideUp();
		num = 0;
	}
})
...
//视频、综艺跳转
$("#lick").click(function(){
	$(".c_listTab").children().removeClass("hide");
...
setInterval(function(){
	...
	j++;
	$(".taobao_nb").children("a").click(function(){
		j=$(this).index()-1;
	})
	if(j==10){
		j=0;
	...
	$(".taobaoImg").children("li").eq(j).addClass("show");
	$(".taobaoImg").children("li").eq(j).siblings().removeClass("show");
	$(".taobao_nb").children("a").eq(j).addClass("active");
	$(".taobao_nb").children("a").eq(j).siblings().removeClass("active");
